        ...
        if ($this->input->method() === 'post') {
            ...
            if ($this->form_validation->run() == TRUE) {
                ...
                if ($existing) {
                    ...
                    // Create data baru (otomatis akan menghapus data kemarin dengan backup)
                    if ($this->Harga_tbs_model->create($data_insert)) {
                        // Hapus data kemarin untuk perusahaan dan kabupaten yang sama
                        // (sudah dilakukan di model, tapi kita bisa cek hasilnya)
                        $deleted_count = $this->Harga_tbs_model->delete_yesterday_data(
                            $data_insert['id_kabupaten'],
                            $data_insert['id_perusahaan'],
                            $data_insert['tanggal'],
                            $user_id
                        );
                        ...
                        if ($deleted_count > 0) {
                            $this->session->set_flashdata('success', 'Harga TBS berhasil ditambahkan. Data kemarin telah dihapus (dengan backup).');
                        } else {
                            $this->session->set_flashdata('success', 'Harga TBS berhasil ditambahkan.');
                        }
                        redirect('admin/harga_tbs');
                    } else {
